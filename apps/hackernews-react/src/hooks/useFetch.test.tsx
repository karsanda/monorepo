...
test('should return idle state when fetch is called', () => {
  ...
  expect(result.all[0]).toEqual({ state: 'idle', data: undefined, error: undefined })
})

test('should return loading state when fetch is in progress', () => {
  const { result } = renderHook(() => useFetch('/item/1'))
  expect(result.current).toEqual({ state: 'loading', data: undefined, error: undefined })
})
...
test('should return fetched state when fetch is success', async () => {
  ...
  expect(result.current).toEqual({ state: 'fetched', data: 'this is dummy data', error: undefined })
})
...
test('should return error state when fetch is error', async () => {
  ...
  expect(result.current).toEqual({ state: 'error', data: undefined, error: errorData })
})